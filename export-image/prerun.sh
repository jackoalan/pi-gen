...
ROOT_SIZE=$(du --apparent-size -s "${EXPORT_ROOTFS_DIR}" --exclude var/cache/apt/archives --exclude var/lib/apt/lists --exclude boot --block-size=1 | cut -f 1)
...
rsync -aHAXx --exclude /var/cache/apt/archives --exclude /var/lib/apt/lists --exclude /boot "${EXPORT_ROOTFS_DIR}/" "${ROOTFS_DIR}/"
